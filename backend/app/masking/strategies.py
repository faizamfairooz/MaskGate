...
class PartialMaskStrategy(MaskingStrategy):
    ...
    def mask(self, value: Any, parameters: Dict[str, Any]) -> Any:
        ...
        return value_str[:visible_chars] + "*" * (len(value_str) - visible_chars * 2) + value_str[-visible_chars:]
...
class MaskingStrategyFactory:
    ...
    _strategies = {
        ...
        'phone_mask': PhoneMaskStrategy(),
        'ssn_mask': SSNMaskStrategy(),
        'credit_card_mask': CreditCardMaskStrategy(),
        'tokenization': TokenizationStrategy(),
        'noise_addition': NoiseAdditionStrategy(),
        'date_mask': DateMaskStrategy(),
